        ...
        <div className="space-y-5">
          ...
          <Section label="ช่องทางติดต่อ">
            ...
            <InfoRow labelWidth="w-36" label="โทรศัพท์" value={
              <a href={`tel:${company.phone}`} className="text-primary-text hover:underline">{company.phone}</a>
            } />
            {company.email && (
              <InfoRow labelWidth="w-36" label="อีเมล" value={
                <a href={`mailto:${company.email}`} className="break-all text-primary-text hover:underline">{company.email}</a>
              } />
            )}
            <InfoRow labelWidth="w-36" label="เว็บไซต์" value={
              <span className="text-primary-text">{company.website}</span>
            } />
          </Section>
