...
import { EMPLOYEES, DEPT_CONFIG, STATUS_CONFIG, EMPLOYEE_HISTORY, type Employee } from "@/lib/employees";
        ...
        <div className="space-y-5">
          ...
          <Section label="ช่องทางติดต่อ">
            <InfoRow label="เบอร์โทรศัพท์" value={
              <a href={`tel:${emp.phone}`} className="text-primary-text hover:underline">
                {emp.phone}
              </a>
            } />
            <InfoRow label="อีเมล" value={
              <a href={`mailto:${emp.email}`} className="break-all text-primary-text hover:underline">
                {emp.email}
              </a>
            } />
          </Section>
