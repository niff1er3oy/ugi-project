...
function EmployeeDetailPanel({ emp, onClose, onDelete, onEdit }: { emp: Employee; onClose: () => void; onDelete: (id: string) => void; onEdit: () => void }) {
  const dept = DEPT_CONFIG[emp.department];
  const status = STATUS_CONFIG[emp.status];
      ...
      <div className="space-y-5">
        <Section label="ข้อมูลส่วนตัว">
          ...
          <InfoRow label="ทีม"          value={
            <span className="inline-flex items-center rounded-full px-2.5 py-[3px] text-[11px] font-semibold" style={{ backgroundColor: dept?.bg, color: dept?.color }}>
              {emp.department}
            </span>
          } />
...
export default function EmployeesPage() {
  ...
  const availableFilterDepts = companyFilter === "all" ? DEPARTMENTS : (COMPANY_DEPARTMENTS[companyFilter] ?? []);
  ...
  }, [search, companyFilter, statusFilter, deptFilter]);
  ...
  return (
    <>
      ...
      <main className="w-full lg:flex lg:h-[calc(100vh-3.5rem)] lg:overflow-hidden">
        ...
        {/* Left: filter + grid (own scroll on desktop) */}
          ...
          <div className="sticky top-14 z-30 border-b border-border bg-background/95 backdrop-blur-md lg:static lg:z-auto lg:shrink-0">
            <div className="space-y-2.5 px-4 pt-3 pb-3">
              <div className="relative">
                <input
                  type="search"
                  placeholder="ค้นหาชื่อ ตำแหน่ง ทีม บริษัท..."
                  value={search}
              ...
              <div className="flex gap-2">
                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value as EmpStatus | "all")}
          ...
          <div className="px-4 py-4 lg:flex-1 lg:overflow-y-auto">
            ...
            {filtered.length > 0 ? (
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-3">
                {filtered.map((emp, i) => (
                  <EmployeeCard key={emp.id} emp={emp} index={i} onSelect={setSelectedId} />
                ))}
              </div>
            ) : (
        ...
        <div className="hidden lg:block lg:w-1/2 lg:overflow-y-auto">
          {(() => {
            ...
            if (panelMode === "add") return <EmployeeAddPanel onClose={() => setPanelMode("detail")} />;
            if (panelMode === "edit" && emp) return <EmployeeEditPanel emp={emp} onClose={() => setPanelMode("detail")} />;
            if (emp) return (
              <EmployeeDetailPanel
                emp={emp}
                ...
                onEdit={() => {
                  if (window.innerWidth >= 1024) setPanelMode("edit");
                  else router.push(`/employees/${emp.id}/edit`);
                }}
              />
            );
