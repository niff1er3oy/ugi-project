...
export default function EmployeeLoginPage() {
  ...
  async function handleEmailLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!snap.exists()) { router.push("/employee/setup-profile"); return; }
      router.push(snap.data().role === "officer" ? "/" : "/me");
    } catch (err: unknown) {
      ...
      <aside className="hidden lg:flex lg:w-[420px] xl:w-[480px] flex-col flex-shrink-0 bg-primary select-none">
        ...
        <div className="flex-1 flex flex-col justify-center px-12">
          ...
          <p className="text-white/50 text-[13.5px] leading-[1.65]">
            ตรวจสอบข้อมูล การอบรม
            <br />
            และงาน Offsite ของคุณ
          </p>
        </div>
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6 py-12 bg-background">
        <div className="w-full max-w-[400px] animate-enter">
          ...
          <p className="mt-7 text-center text-[13px] text-muted">
            ยังไม่มีบัญชี?{" "}
            <Link href="/employee/register" className="font-medium text-primary-text hover:underline">สมัครใหม่</Link>
          </p>
          <p className="mt-3 text-center text-[13px] text-muted">
            เป็นเจ้าหน้าที่?{" "}
            <Link href="/login" className="font-medium text-primary-text hover:underline">เข้าสู่ระบบเจ้าหน้าที่</Link>
          </p>
