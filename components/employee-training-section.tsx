...
export default function EmployeeTrainingSection({ employeeId }: { employeeId: string }) {
  ...
  useEffect(() => {
    fetchTrainings().then((all) =>
      setRecords(all.filter((r) => r.participants.includes(employeeId))),
    );
  }, [employeeId]);
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="truncate text-[13px] font-medium text-ink leading-snug">{record.title}</p>
