...
import { STATUS_CONFIG, type HistoryEntry } from "@/lib/employees";

export default function EmployeeHistorySection({
  history,
}: {
          ...
          <div key={i} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex flex-1 items-center justify-between gap-3 py-0.5">
              <div className="flex items-center gap-2">
                <span
                  className={`inline-flex items-center rounded-full px-2.5 py-[3px] text-[11px] font-semibold ${cfg.bg} ${cfg.text}`}
                >
                  {cfg.label}
                </span>
                {entry.note && (
                  <span className="text-[12px] text-muted">{entry.note}</span>
                )}
              </div>
